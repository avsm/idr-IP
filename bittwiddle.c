...
INT32 getBits(INT32 num, word32 startbit, word32 endbit) {
    int* n = EMALLOC(sizeof(int));
    word32 v = *(int*)num;
    *n = (v << startbit) >> (startbit + (32-endbit));
    return n;
}
...
// We assume the number doesn't overflow (so we only need to worry about 
// endbit). It's a safe assumption if we have the right type Idris-side.
...
INT32 setBits(INT32 num, word32 startbit, word32 endbit, word32 newval) {
    int* n = EMALLOC(sizeof(int));
    word32 v = *(int*)num;
    // Clear the bits in v from startbit to endbit.
    word32 two_m = (0xffffffff << (32-startbit)) ^ 0xffffffff;
    word32 two_n = (0xffffffff << (32-endbit)) ^ 0xffffffff;
    ...
    // Put the newval in the right place (from startbit to endbit, all other
    // bits zero).
    word32 nv = newval << (32-endbit);
...
int main() {
    ...
    INT32 test1 = mkInt32(0xdeadbeef);
    word32 test2 = 0xddab;

    printf("%x 8-24th bits are %x\n", getInt(test1), 
	                              getInt(getBits(test1, 8, 24)));

    INT32 test3 = setBits(test1, 9, 25, test2);

    printf("%x replaced 9-25th bits gives %x\n", getInt(test1), getInt(test3));

    printf("%x 9-25th bits are %x\n", getInt(test3), 
	                              getInt(getBits(test3, 9, 25)));
}
*/
